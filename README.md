...
This is not an exhaustive list, and even if some workarounds can be somewhat effective, the underlying design *principles* are suboptimal.
...
3. All new outputs to the address must contain the proper datum:
    - The new `SwapPrice` datum field must be > 0.
...
Currently, swap addresses are unique for every trading pair. This necessitates many distinct Beacon UTxOs (and associated deposits), even if users are offering the same asset in exchange for varying assets. Future versions of Cardano-Swaps will instead utilize "Offer-based" swap addresses, which are unique only to the asset being offered. The desired "exchange" asset will be specified in the datum of every "Swap" UTxO at the address. This will dramatically lower Beacon UTxO deposit fees, as only one such UTxO will be needed per "Offer" asset, per user. This will also improve querying efficiency; instead of having to lookup a specific trading pair, users can lookup all swap addresses offering Token "ABC", and then filter through the desired exchange asset(s).

> **Note**
> This feature will require the introduction of a new Beacon Token.


### Redundant Script Executions
Currently, plutus validator scripts must be executed once for *every* UTxO being consumed from a script address (this is the case for Plutus as a whole, not just Cardano-Swaps). If multiple "Swap" UTxOs from the same address are consumed as inputs, the validator must execute for each of them, even though their spending logic is exactly the same. These executions are "redundant", in a sense, resulting in a (low severity) composability bottleneck. Addressing this would likely require significant changes to Cardano's transaction structure, which is unnecessary at this point in time.  [Present and future discussions with the IOG team](https://github.com/cardano-foundation/CIPs/pull/418) will help determine if this feature is indeed warranted.

### Swap Collisions
Recall the composition example above. What would happen if another user simultaneously submits a transaction using *one* of the same "Swap" UTxOs as an input? Whichever transaction is processed by (most) nodes first will succeed, and the other will fail entirely. Arbitragers/market-makers will be in constant competition with one another to execute the most profitable swaps at any point in time, and must strike a balance between simple swaps and complex composed swaps. The greater the input complexity of a large swap, the higher chance that one of the inputs is spent in the interim, making the whole transaction fail.
