...
module Test.OneWaySwap.UpdateSwap
  ( 
    -- * Scenarios Tested
    ...
    emptyTxParams
      ...
      , outputs =
          [ Output
              ...
              , outputDatum = OutputDatum $ toDatum swapDatum2{swapPrice = unsafeRatio 10 1}
              , outputReferenceScript = toReferenceScript Nothing
              }
          ]
      , referenceInputs = [mintRef,spendRef]
...
benchTest3 number = do
  ...
  -- Initialize scenario
  (mintRef,spendRef) <- initializeReferenceScripts 
  mintTestTokens sellerWallet 100_000_000 $ zip (drop 60 assetNames) (repeat 1000)
